...
class BEACON(object):
    ...
    but is modified so it can deal with M=K case.
    """
    ...
    def update(self, play, obs):
        ...
        # If we are in Initialization.Verification state
            ...
            # Increase number of collisions, this will be equal to M at the end of the round.
            self.collisions += col
            ...
            if self.state_round == 2 * self.K - 2:
                ...
                # state_round are used as a clock within one phase
                self.state_round = 0
                self.phase = self.INITIALIZATION
                return
        # INITIALIZATION is the first exploration phase where players sample K arms uniformly
        elif self.phase == self.INITIALIZATION:
