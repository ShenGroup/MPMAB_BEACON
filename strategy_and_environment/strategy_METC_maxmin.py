...
class VaryMeanMETCElim_max_min(PlayerStrategy):
    """
    METC is modified to solve minimal function. Details can be found in the appendix
    """
        ...
        # If we are in Initialization.Verification state
            ...
            if self.state_round == 2 * self.K:
                ...
                if self.relative_position == 1:
                    self.is_leader = True
                self.Mp = self.M
                #print(self.M, self.relative_position)
                self.collisions = 0
            ...
            if self.state_round == len(self.arm_to_explore) * np.ceil(2**(self.p**self.c)):
                ...
                #epsilonp = np.sqrt(np.log(self.M**2*self.K*self.T**2)/2/self.Tp)
                #self.Qp = int(np.ceil(-np.log2(0.1*epsilonp)))
    ...
    def get_optimal_matching(self, pair=None):
        ...
        xx = list(itertools.permutations(L, int(self.M)))
        tmp = []
        optimal_value = -333
        if pair == None:
            for i in range(len(xx)):
                if np.prod(self.record_player_stat[list(range(self.M)), xx[i]]) > optimal_value:
                    optimal_value = np.prod(self.record_arm_empirical_means[list(range(self.M)), xx[i]])
                    tmp = xx[i]
        else:
            for i in range(len(xx)):
                if xx[i][pair[0]] != pair[1]:
                    continue
                if np.prod(self.record_player_stat[list(range(self.M)), xx[i]]) > optimal_value:
                    optimal_value = np.prod(self.record_arm_empirical_means[list(range(self.M)), xx[i]])
                    tmp = xx[i]
    ...
    def leader_update_active_set(self):
        ...
        tmp = 1
        for i in range(self.M):
            if self.record_arm_empirical_means[i, pi_star[i]] - 1.1*epsilonp < 0:
                tmp = 0
        for i in range(self.M):
            for j in range(self.K):
                if self.active_matching[i][j] == 1:
                    ...
                    if np.prod(self.record_arm_empirical_means[list(range(self.M)), hat_pi]+1.1*epsilonp) > tmp * np.prod(self.record_arm_empirical_means[list(range(self.M)), pi_star]-1.1*epsilonp):
                        if flag[i][j] == 1:
                            continue
